...
export default function ProductListingPage({ onAddToCart, wishlist, onToggleWishlist, showNotif }) {
  ...
  let filtered = products.filter(p => {
    const matchHewan = !hewan || p.hewan === hewan;
    const matchSearch = !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.kat.toLowerCase().includes(search.toLowerCase());
    const matchMin = !minPrice || p.price >= Number(minPrice);
    const matchMax = !maxPrice || p.price <= Number(maxPrice);
    return matchHewan && matchSearch && matchMin && matchMax;
  });

  if (sort === 'harga-asc') filtered = [...filtered].sort((a, b) => a.price - b.price);
  else if (sort === 'harga-desc') filtered = [...filtered].sort((a, b) => b.price - a.price);
  else if (sort === 'rating') filtered = [...filtered].sort((a, b) => b.rating - a.rating);
  else if (sort === 'terlaris') filtered = [...filtered].sort((a, b) => b.sold - a.sold);
...
const s = {
  ...
  priceInput: { border: '1px solid #e5e7eb', borderRadius: '8px', padding: '6px 10px', fontSize: '13px', fontFamily: "'Nunito', sans-serif", width: '100%', outline: 'none' },
  ...
  prodSearchInput: { flex: 1, border: '1px solid #e5e7eb', borderRadius: '30px', padding: '10px 20px 10px 40px', fontSize: '14px', fontFamily: "'Nunito', sans-serif", outline: 'none', background: '#fff' },
