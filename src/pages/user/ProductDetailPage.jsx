...
import { Heart, ShoppingCart, Truck, ImageOff, RefreshCw, Trophy, Tag, Sparkles, Frown } from 'lucide-react';
...
export default function ProductDetailPage({ onAddToCart, wishlist = [], onToggleWishlist, showNotif }) {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await shopProductsAPI.getById(id);
        if (res.data.success) {
          const p = res.data.data;
          setProduct(p);
          // fetch related
          const relatedRes = await shopProductsAPI.getAll({ hewan: p.hewan });
          if (relatedRes.data.success) {
            setRelated(relatedRes.data.data.filter(x => x._id !== p._id).slice(0, 4));
          }
        }
      } catch (err) {
  ...
  if (loading) return (
    <div style={{ textAlign: 'center', padding: '80px' }}>
      <p style={{ fontSize: '18px', marginBottom: '16px' }}>Memuat produk...</p>
    </div>
  );

  if (!product) return (
    <div style={{ textAlign: 'center', padding: '80px' }}>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}><Frown size={48} color="#9ca3af" /></div>
  ...
  const discPct = product.salePrice ? Math.round((product.salePrice - product.price) / product.salePrice * 100) : 0;
  const thumbs = [null, null, null, null];

  const handleAddToCart = async () => {
    const res = await addToCart(product._id || product.id, qty);
    if (res?.success) {
      showNotif?.(`${qty}x ${product.title || product.name} ditambahkan ke keranjang!`);
    } else if (res?.message) {
    ...
    <div>
      ...
      <div style={s.prodHero}>
        <div className="container">
          <div style={s.breadcrumb}>
            ...
            <span style={s.breadSep}>›</span>
            <span style={s.breadCurrent}>{product.title || product.name}</span>
          </div>
        </div>
      </div>
      ...
      <div style={s.detailWrap}>
        <div className="container" style={{ display: 'flex', gap: '40px', alignItems: 'flex-start' }}>
          {/* Gallery */}
          <div style={s.gallery}>
            <div style={{ ...s.galleryMain, overflow: 'hidden' }}>
              {product.image ? (
                <img src={product.image} alt={product.title || product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
            ...
            <div style={s.galleryThumbs}>
              {thumbs.map((_, i) => (
                <div key={i} style={{ ...s.galleryThumb, overflow: 'hidden', ...(thumbIdx === i ? s.galleryThumbActive : {}) }} onClick={() => setThumbIdx(i)}>
                  {i === 0 && product.image ? (
          ...
          <div style={s.detailInfo}>
            <div style={s.detailBadges}>
              ...
              {product.salePrice > 0 && <span style={s.badgeDiscount}><Tag size={12} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} />Diskon {discPct}%</span>}
              {product.isNewProduct && <span style={s.badgeNew}><Sparkles size={12} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} />Baru</span>}
            </div>
            <div style={s.detailName}>{product.title || product.name}</div>
            <div style={s.detailRating}>
              <span style={{ color: '#f59e0b', fontSize: '16px' }}>{starsHTML(product.rating || 0)}</span>
              <span style={{ fontSize: '14px', color: '#1a1a2e' }}>{product.rating || 0}/5</span>
              <span style={{ fontSize: '14px', color: '#2d7a4f', cursor: 'pointer' }} onClick={() => setActiveTab('reviews')}>({product.reviews || 0} ulasan)</span>
            </div>
            <div style={s.detailPriceRow}>
              <span style={s.detailPrice}>{fmtPrice(product.price)}</span>
              {product.salePrice > 0 && <span style={s.detailPriceOld}>{fmtPrice(product.salePrice)}</span>}
              {product.salePrice > 0 && <span style={s.detailDiscPct}>{discPct}% OFF</span>}
            </div>
            <div style={s.detailMeta}>
              <div><label style={s.metaLabel}>Stok</label><span style={s.metaVal}>{product.stock} tersedia</span></div>
              <div><label style={s.metaLabel}>Kategori</label><span style={s.metaVal}>{product.kat || product.category}</span></div>
              ...
              <div><label style={s.metaLabel}>Terjual</label><span style={s.metaVal}>{product.sold || 0}+ item</span></div>
            </div>
            <div style={s.qtyRow}>
              ...
              <span style={{ fontSize: '13px', color: '#6b7280' }}>Sisa {product.stock} item</span>
            </div>
            ...
            {activeTab === 'desc' && <p style={{ fontSize: '14px', color: '#1a1a2e', lineHeight: 1.8 }}>{product.description || product.desc}</p>}

            {activeTab === 'reviews' && (
              <div>
                <div style={s.reviewSummary}>
                  <div style={s.reviewBig}>
                    <div style={s.reviewBigNum}>{product.rating || 0}</div>
                    <div style={{ fontSize: '24px', color: '#f59e0b', margin: '4px 0' }}>★★★★★</div>
                    <div style={{ fontSize: '13px', color: '#6b7280' }}>dari {product.reviews || 0} ulasan</div>
                  </div>
                  <div style={{ flex: 1 }}>
            ...
            {activeTab === 'info' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                {[['Nama Produk', product.title || product.name], ['Kategori', `${product.kat || product.category} → ${product.subkat || ''}`], ['Jenis Hewan', (product.hewan || '').charAt(0).toUpperCase() + (product.hewan || '').slice(1)], ['Berat', '1-5 kg']].map(([k, v]) => (
                  <tr key={k}>
                    <td style={{ padding: '8px', borderBottom: '1px solid #e5e7eb', color: '#6b7280', width: '140px' }}>{k}</td>
                    <td style={{ padding: '8px', borderBottom: '1px solid #e5e7eb', color: '#1a1a2e' }}>{v}</td>
                  </tr>
                ))}
              </table>
            )}
      ...
      <div className="section section-alt">
        <div className="container">
          ...
          <div style={s.relatedGrid}>
            {related.map(p => <ProductCard key={p._id || p.id} product={p} onAddToCart={onAddToCart} wishlist={wishlist} onToggleWishlist={onToggleWishlist} showNotif={showNotif} />)}
          </div>
        </div>
      </div>
      ...
      <div style={s.stickyCart}>
        <div>
          <div style={{ fontWeight: 600, fontSize: '15px', color: '#1a1a2e' }}>{product.title || product.name}</div>
...
const s = {
  ...
  qtyNum: { width: '48px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '16px', color: '#1a1a2e', borderLeft: '1px solid #e5e7eb', borderRight: '1px solid #e5e7eb' },
